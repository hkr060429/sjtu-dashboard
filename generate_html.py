...
def render_page(data: dict) -> str:
    ...
    # ── 邮件列表HTML（避免在 f-string 中用反斜杠） ──
    mail_items_html = ""
    for e in m.get("emails", [])[:5]:
        subj = e.get("subject", "")
        dt = e.get("date", "")
        mail_items_html += f'              <div class="mail-list-item"><span class="mail-subject">{subj}</span><span class="mail-date">{dt}</span></div>\n'
    ...
    for s in shuiyuan_list:
        if "error" in s:
            shuiyuan_cards += f'<div class="shuiyuan-item"><span class="shuiyuan-title" style="color:#999;">⚠️ {s["error"]}</span></div>'
            continue
        sid = s.get("id", "")
        title = s.get("title", "")
        url = s.get("url", "")
        ...
        <a class="shuiyuan-item" href="{url}" target="_blank">
          ...
          <span class="shuiyuan-meta">💬 {posts}  👁️ {views}  {('❤️ '+str(likes)) if likes else ''}</span>
        </a>"""
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .mail-subject {{ color: #333; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 180px; }}
        ...
        <div class="brief-grid">
          ...
          <div class="brief-item">
            ...
            <div class="brief-text">
              ...
              <div class='value days'>{c.get("days_left", "?")}<span style='font-size:14px;color:#888;font-weight:400;'> 天</span></div>
          ...
          <div class="brief-item" style="flex-direction:column;align-items:stretch;">
            <div style="display:flex;align-items:center;gap:12px;">
              ...
              <div class="brief-text">
                ...
                <div class='value' style='font-size:20px;'>{m.get("count",0)}<span style='font-size:14px;color:#888;font-weight:400;'> 封</span></div>
                {"<div class='mail-error'>"+m.get("error","")+"</div>" if has_mail_err else ""}
              </div>
            </div>
            <div class="mail-list">
{mail_items_html}
    ...
    <div class="card">
      ...
      <div class="card-body">
        {news_cards if news_cards else '<div style="color:#ccc;text-align:center;padding:20px;">暂无新闻</div>'}
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        {shuiyuan_cards if shuiyuan_cards else '<div style="color:#ccc;text-align:center;padding:20px;">暂无数据</div>'}
      </div>
    </div>
  </div>
...
def main():
    if not os.path.isfile(DATA_PATH):
        print(f"⚠️ {DATA_PATH} 不存在，请先运行 fetch_all.py")
        return
